...
frish {
  ...
  TopLevel =
   ...
   BuiltinPhrase =
     ...
     | "%funcall" Primary Actuals -- funcall

     | "%inCompilationState" -- incompilationstate
...
Defsubr : function (_4,ident,StatementBlock,line,) {
enter_rule ("Defsubr");
...
enter_rule ("BuiltinPhrase_push");
    set_return (`State.S.append (${exp.rwr ()})`);
...
BuiltinPhrase_lookup : function (_,lp,dict,_comma1,str,rp,) {
enter_rule ("BuiltinPhrase_lookup");
    set_return (`Lookup (${dict.rwr ()}, ${str.rwr ()})`);
return exit_rule ("BuiltinPhrase_lookup");
},
...
enter_rule ("BuiltinPhrase_unrecognized");
    set_return (`${_.rwr ()} ⎝ error - unrecognized builtin (with given arguments) ⎠ ${args.rwr ().join ('')} `);
